...
//구조체는 카메라 변환 행렬과 투영 변환 행렬을 나타낸다.
struct VS_CB_CAMERA
{
...
public:
	...
	//void GenerateViewMatrix(D3DXVECTOR3 d3dxvEyePosition, D3DXVECTOR3 d3dxvLookAt, D3DXVECTOR3 d3dxvUp);//카메라 변환행렬
	void GenerateProjectionMatrix(float fNearPlaneDistance, float fFarPlaneDistance, float fAspectRatio, float fFOVAngle);//투영변환 행렬

	//상수 버퍼를 생성하고 내용을 갱신하는 멤버 함수를 선언한다.
	void CreateShaderVariables(ID3D11Device *pd3dDevice);//상수버퍼를 생성
	void UpdateShaderVariables(ID3D11DeviceContext *pd3dDeviceContext);// 그 내용을 갱신
...
//CCamera 클래스에서 파생 클래스 CThirdPersonCamera를 다음과 같이 선언한다.
