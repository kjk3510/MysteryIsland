...
{
	//쉐이더는 지형, 빨강색, 녹색, 파란색 인스턴싱 쉐이더를 생성한다.
	m_nShaders = 2;
	...
	CInstancingShader *pRedInstancedObjectsShader = new CInstancingShader();
	pRedInstancedObjectsShader->CreateShader(pd3dDevice);
	pRedInstancedObjectsShader->BuildObjects(pd3dDevice, GetTerrain(), pRedMaterial, 0);
	m_ppShaders[1] = pRedInstancedObjectsShader;
...
void CScene::CreateShaderVariables(ID3D11Device *pd3dDevice)
{
	...
	m_pLights->m_pLights[1].m_fRange = 250.0f;
