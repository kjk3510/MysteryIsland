...
//Direct3D 디바이스의 뷰-포트를 설정하기 위한 함수이다. 04의 framework에 있던 함수를 그데로 가져온다.
...
void CCamera::CreateShaderVariables(ID3D11Device *pd3dDevice)
{
	...
	pd3dDevice->CreateBuffer(&bd, NULL, &m_pd3dcbCamera);//그 상수버퍼의 인터페이스를 pd3dcdCamera변수로 받는다
}//상수버퍼 생성
void CCamera::UpdateShaderVariables(ID3D11DeviceContext *pd3dDeviceContext)
{
	...
	*/
	...
	pd3dDeviceContext->Map(m_pd3dcbCamera, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3dMappedResource);//리소스, 서브리소스, 타입, 플레그, 복사 당하는 리소스

	VS_CB_CAMERA *pcbViewProjection = (VS_CB_CAMERA *)d3dMappedResource.pData;//카메라 변환 행렬 복사해서 VS_CB_CAMERA 형 변수에 집어 넣음.
	...
	pd3dDeviceContext->VSSetConstantBuffers(VS_SLOT_CAMERA, 1, &m_pd3dcbCamera);//카메라 슬롯에 슬롯 1개만큼, m_pd3dcdCamera로 갱신.
}
//상수버퍼 갱신
