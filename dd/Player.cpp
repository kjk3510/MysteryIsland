...
void CPlayer::Update(float fTimeElapsed)
{
	...
	if (m_d3dxvPosition.x <= 8) m_d3dxvPosition.x = 8;
	if (m_d3dxvPosition.z <= 8) m_d3dxvPosition.z = 8;
	if (m_d3dxvPosition.x >= 760) m_d3dxvPosition.x = 760;
	if (m_d3dxvPosition.z >= 760) m_d3dxvPosition.z = 760;

	if (m_pCollisionMap && m_pCollisionMap->GetHeight(m_d3dxvPosition.x, m_d3dxvPosition.z, false) > 0)
	{
		Move(-m_d3dxvVelocity, false);
	}
