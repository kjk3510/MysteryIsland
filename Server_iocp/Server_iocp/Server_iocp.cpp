...
void ProcessPacket(int now_ID, unsigned char * packet) {
	...
	sc_packet_put_player pos_packet;
	
	pos_packet.id = now_ID;
	pos_packet.size = sizeof(sc_packet_put_player);
	pos_packet.type = SC_PUT_PLAYER;
...
void Accept_Thread()
{
	...
	while (true) {
		...
		{
			cout << "asdf" << endl;
			SendPacket(0, reinterpret_cast<unsigned char*>(&put_player_pacekt));
		}
*/
		//// 시야 처리
		//for (int i = 0; i < MAX_USER; ++i) {
