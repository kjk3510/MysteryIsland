...
HWND main_handle = NULL; // 윈도우핸들
HINSTANCE main_instance = NULL; // 인스턴스
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			if (!mAppPaused)
			{
				...
				//	std::cout << "networkevent eroo" << std::endl;
				////이벤트 걸리면 아래 if문 실행
...
LRESULT D3DApp::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	/*case WM_KEYDOWN: {
		...
		cs_packet_player *my_packet = reinterpret_cast<cs_packet_player *>(send_buffer);
		my_packet->size = sizeof(my_packet);
		send_buf.len = sizeof(my_packet);
		DWORD iobyte;
		if (0 != x) {
			if (1 == x) my_packet->type = CS_RIGHT;
			else my_packet->type = CS_LEFT;
			int ret = WSASend(sock, &send_buf, 1, &iobyte, 0, NULL, NULL);
			if (ret) {
...
bool D3DApp::InitMainWindow()
{
	...
	main_handle = mhMainWnd;
	main_instance = mhAppInst;
	...
	WSAAsyncSelect(sock, main_handle, WM_SOCKET, FD_CLOSE | FD_READ);
	std::cout << "asyncselect complete" << std::endl;
